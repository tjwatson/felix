...
public class BundleWiringLastModified implements Serializable {
    ...
    private final Map<Long, Long> lastModifieds = new HashMap<Long, Long>();
    ...
    public BundleWiringLastModified(BundleWiring hostWiring)
    {
        ...
        if (hostWiring.isCurrent())
        {
            ...
            lastModifieds.put(hostWiring.getBundle().getBundleId(),
                hostWiring.getBundle().getLastModified());
        }
        else
        {
            // use a unique negative id to indicate not current
            lastModifieds.put(nextNotCurrentID.getAndDecrement(),
                hostWiring.getBundle().getLastModified());
        }
        for (BundleWire hostWire : hostWiring.getProvidedWires(
            HostNamespace.HOST_NAMESPACE))
        {
            ...
            lastModifieds.put(hostWire.getRequirer().getBundle().getBundleId(),
                hostWire.getRequirer().getBundle().getLastModified());
        }
    }
